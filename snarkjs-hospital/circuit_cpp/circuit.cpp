...
void VerifyPatient_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyPatient_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 22;}
...
LessThan_1_create(mySignalStart+2,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
uint cmp_index_ref = 0;
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 11
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
VerifyPatient_2_create(1,0,ctx,"main",0);
VerifyPatient_2_run(0,ctx);
}
