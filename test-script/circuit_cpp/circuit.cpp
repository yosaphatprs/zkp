...
void VerifyPatient_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyPatient_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "VerifyPatient";
ctx->componentMemory[coffset].signalStart = soffset;
...
int cmp_index_ref_load = -1;
{
std::string new_cmp_name = "ageCheck";
LessThan_1_create(mySignalStart+5,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "vaccineCheck";
IsEqual_3_create(mySignalStart+26,2+ctx_index+1,ctx,new_cmp_name,myId);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 13
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 4],&circuitConstants[2]); // line circom 23
}}
...
VerifyPatient_4_create(1,0,ctx,"main",0);
VerifyPatient_4_run(0,ctx);
}
