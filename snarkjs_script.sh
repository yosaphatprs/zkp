...
# Get folder name from env or prompt
if [ -z "$FOLDER_NAME" ]; then
    echo -n "Enter name for new folder: "
    read folder_name
else
    folder_name="$FOLDER_NAME"
    echo "Using folder name from .env: $folder_name"
fi
...
if [ -z "$CIRCUIT_FILE" ]; then
    echo -n "Enter circuit filename from circuit/ folder: "
    read circuit_file
else
...
circuit_source="../circuit/$circuit_file"
if [ -f "$circuit_source" ]; then
    echo "Copying circuit file from circuit/$circuit_file..."
